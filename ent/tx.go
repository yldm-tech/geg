...
func (tx *Tx) init() {
	tx.ETCRecord = NewETCRecordClient(tx.config)
	tx.ETCSyncRecord = NewETCSyncRecordClient(tx.config)
	tx.ETCUser = NewETCUserClient(tx.config)
}
...
// applies a query, for example: ETCRecord.QueryXXX(), the query will be executed
